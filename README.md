# Tableau Sales & Customer Insights Dashboard

## Project Overview
This project focuses on analyzing sales performance and customer behavior using Tableau dashboards. 

The objective was to transform raw sales data into interactive dashboards that help stakeholders understand key business metrics and trends.

<img width="1199" height="799" alt="Sales Dashboard" src="https://github.com/user-attachments/assets/85ecd062-3ccd-4729-a631-5ad69b18449e" />
<img width="1199" height="799" alt="Customer Dashboard (1)" src="https://github.com/user-attachments/assets/ffc6f7fc-52f9-430b-b504-6de3a1ddf868" />

## Dashboards Created

### 1. Sales Performance Dashboard
This dashboard provides an overview of sales performance metrics and trends.

Key features:
- Year-over-year comparison of Sales, Profit, and Quantity
- Monthly sales trend analysis
- Product subcategory sales comparison
- Weekly sales and profit performance against average

### 2. Customer Insights Dashboard
This dashboard analyzes customer behavior and contribution.

Key features:
- Customer growth and order trends
- Distribution of customers based on number of orders
- Identification of high-value customers
- Top 10 customers ranked by profit

## Tools Used
- Tableau
- Data Visualization
- KPI Analysis
- Dashboard Design

## Key Business Insights
- Certain product subcategories contribute higher sales but lower profit margins.
- A small group of customers contributes a significant portion of total profit.
- Weekly trend analysis helps identify performance fluctuations during the year.

These insights help sales teams identify profitable product segments, track customer contribution to revenue, and monitor sales performance trends to support strategic planning.

## Dashboard Link

Interactive dashboard available here:
https://public.tableau.com/app/profile/amneet.kaur2844

## Structure
```
sales-and-customer-behavior-analytics
│
├── dashboards
│   └── Sales and Customer Dashboard.twbx
│
├── dataset
│   ├── Customers.csv
│   ├── Location.csv
│   ├── Orders.csv
│   └── Products.csv
│
├── images
│   ├── Customer Dashboard (1).png
│   └── Sales Dashboard.png
│
├── insights
│   └── key_findings.md
│
└── README.md
```

## Author
Amneet Kaur
Aspiring Data Analyst | SQL | Tableau | Power BI | Python
